		...
		>
				...
				<div className="flex gap-2 mb-4">
					{[
						...
						<button
							key={opt.value}
							type="button"
							className={`p-2 rounded border text-xs font-medium transition-all duration-150 bg-opacity-35 focus:outline-none ${
			...
			<div>
				...
				<div className="flex gap-2 mb-3">
					<input
						type="text"
						placeholder="Search for backgrounds (e.g., nature, sunset, flowers)"
						value={searchKeyword}
						onChange={(e) => setSearchKeyword(e.target.value)}
						onKeyPress={handleKeyPress}
						className="flex-1 px-3 sm:py-0 text-xs sm:text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
					/>
					<button
						...
						<div className="flex gap-2 flex-wrap">
							{searchedImages.map((img, idx) => (
								<button
									key={`search-${img}`}
									type="button"
									className={`rounded overflow-hidden border-2 transition-all duration-150 focus:outline-none ${
					...
					additionalImages.length > 0 && (
						<div className="mb-6">
							<div className="mb-2 font-semibold text-sm text-gray-700 dark:text-gray-200">
								...
								{Array.isArray(ImageKeyword) ? ImageKeyword[0] : ImageKeyword}
							</div>
							<div className="flex gap-2 flex-wrap">
